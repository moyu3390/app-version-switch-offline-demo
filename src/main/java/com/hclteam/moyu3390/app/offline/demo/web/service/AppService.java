...
public class AppService {
    ...
    public Object publishApp(String appId, String version) {
        ...
        AppInfo appInfo = ProcessorManager.loadVersionProcessor(appId, ProcessorManager.jarDir, ProcessorManager.packagePath, version);
        if (isRunning) {
            try {
                // 模拟加载应用其他逻辑耗时
                Thread.sleep(2000);
...
//            AppThreadCaller.call(task);
            ...
            if (Objects.isNull(future) || future.isDone()) {
                ...
                //  开启线程计时
                AppOfflineRunnable task = new AppOfflineRunnable(1 * 10 * 1000, appInfo);
                // 清理线程等待倒计时结束，开始卸载旧应用的classloader和已加载的实例信息
                // 把新应用暂存的信息 保存到应用信息容器中  完成旧应用的替换
                System.err.println("新应用的发布任务，" + appId);
